<?php
  ...
  if ($_GET['token']) {
    ...
    $product = $obj->product_store_products_search('approved', '', null, null, 1, 1);

    // We want to make sure the image is not bigger than the box
    $img_size = ($width < $height) ? "width='".$width."px'" : "height='".($height - 30)."px'";
?>
...
<h5><?php echo $product->title; ?></h5>
    <div style='float: left;'><img <?php echo $img_size; ?> src='<?php echo $product->{'image-url'}; ?>'></div>
